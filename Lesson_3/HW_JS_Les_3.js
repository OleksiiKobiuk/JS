...
for (const user in usersWithId) {
    const id = usersWithId[user].id
    for (const citiesKey in citiesWithId) {
        const address_id = citiesWithId[citiesKey].user_id
        if (id === address_id) {
           usersWithId[user].address = citiesWithId[citiesKey];
        }
    }
}
